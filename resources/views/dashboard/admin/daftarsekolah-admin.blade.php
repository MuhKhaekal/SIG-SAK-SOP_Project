...
@section('content')
<h1 class="text-3xl font-bold bg-yellow-400 w-fit px-5 py-2  rounded-md shadow-md text-primary" data-aos="fade-down">DAFTAR SEKOLAH</h1>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs uppercase bg-primary  text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">Alamat Sekolah</th>
            ...
            @foreach ($schools as $school)
                <tr>
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap ">
                        <a href="" class="hover:underline">{{ $school->nama_sekolah }}</a>
                    </t>
                    <td class="px-6 py-4">{{ $school->npsn }}</td>
                    <td class="px-6 py-4">{{ $school->akreditasi }}</td>
                    <td class="px-6 py-4">{{ $school->p_pd + $school->l_pd }}</td>
                    <td class="px-6 py-4">{{ $school->alamat }}</td>
                    <td class="px-6 py-4 flex">
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('daftarsekolah.edit', ['daftarsekolah'=>$school->id]) }}" class="font-medium text-blue-600 hover:underline me-4" class="flex items-center">Edit</a>
                        </div>
                        <form action="{{ route('daftarsekolah.destroy', ['daftarsekolah'=>$school->id]) }}" method="POST" class="flex items-center">
                            @csrf
                            @method('DELETE')
...
<div class="mt-10" data-aos="fade">
    <a href="{{ route('daftarsekolah.create') }}" class="bg-blue-500 text-white px-3 py-2 rounded-md mt-5 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 shadow-lg">+ Tambah Data Sekolah</a>
</div>
